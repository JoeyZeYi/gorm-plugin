...
func (b *baseStore[T]) ListQuery(ctx context.Context, sql string, args []interface{}, order string) ([]*T, error) {
	list := make([]*T, 0)
	db := b.DB(ctx)
...
func (b *baseStore[T]) FirstQuery(ctx context.Context, sql string, args []interface{}, order string) (*T, error) {
	value := new(T)
	db := b.DB(ctx)
	if order != "" {
		db = db.Order(order)
	}
	err := db.Where(sql, args...).Take(value).Error
	if err != nil {
		return nil, err
	}
...
func (b *baseStore[T]) ListPage(ctx context.Context, sql string, pageNum, pageSize int, args []interface{}, order string) ([]*T, int, error) {
	list := make([]*T, 0)
	db := b.DB(ctx).Where(sql, args...)
	if order != "" {
		db = db.Order(order)
	}
	count := int64(0)
	err := db.Model(list).Count(&count).Limit(pageSize).Offset((pageNum - 1) * pageSize).Find(&list).Error
	if err != nil {
		return nil, 0, err
	}
